...
function Header() {
    ...
    return (
        <div className='sticky top-0 z-50 bg-white border-b shadow-md'>
            <div className='flex items-center justify-between grid-cols-3 px-2 py-4 mx-auto space-x-2 md:grid max-w-7xl md:px-4'>
                ...
                <div className='flex-1 max-w-[400px] w-full'>
                    <div className='flex justify-between border border-gray-300 rounded-full '>
                        <input onFocus={handleCalendar} onChange={(e) => setSearchInput(e.target.value)} type="text" id='search' placeholder='Start your search' className='flex-1 px-2 text-xs bg-transparent rounded-full placeholder:text-gray-600 w-7 sm:w-auto focus:outline-none' />
            ...
            {
                isOpen && (
                    <DatePicker cancel={() => setIsOpen(false)} />
                )
            }
        </div>
    )
